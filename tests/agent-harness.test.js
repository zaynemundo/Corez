...
function makeHarness({ provider, store, repositoryRunner, maxRetryWaitMs = 0 } = {}) {
  return new AgentHarness({
    ...
    providerChain: provider || {
      async generate() {
        return { status: 'completed', content: 'answer', toolCalls: [], provider: 'test', model: 'muse-spark-1.3-contributor' };
      }
    },
    ...
    const harness = makeHarness({
      ...
      provider: {
        async generate() {
          ...
          return { status: 'completed', content: 'recovered answer', toolCalls: [], provider: 'opencode-go', model: 'muse-spark-1.3-contributor' };
        }
      }
    });
