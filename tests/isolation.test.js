...
function immediateProvider(content) {
  return {
    async generate() {
      return { status: 'completed', content, toolCalls: [], provider: 'test', model: 'mimo-v2.5' };
    }
  };
}
  ...
  it('two users run concurrent tasks with never-crossing messages and workspaces', async () => {
    const harness = harnessFor({
      async generate({ messages }) {
        ...
        return { status: 'completed', content: userMessages[userMessages.length - 1] || '', toolCalls: [], provider: 'test', model: 'mimo-v2.5' };
      }
    });
