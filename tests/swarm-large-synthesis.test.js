  ...
  return {
    ...
    async generate({ messages }) {
      ...
      const content = chunkCount > 0
        ...
        : 'Final merged answer';
      return { status: 'completed', content, toolCalls: [], provider: 'mock', model: 'deepseek-v4.1-flash' };
    }
  };
}
...
describe('1,001-workstream hierarchical synthesis', () => {
  ...
  it('persists wave state and resumes after a restart without re-running completed waves', async () => {
    ...
    await first.synthesize({ outputs: makeOutputs(1001), prompt: 'wave build', model: 'deepseek-v4.1-flash' });
    ...
    const continuation = await resumed.resume({ prompt: 'wave build', model: 'deepseek-v4.1-flash' });
