...
export function classifyTaskCategory(task) {
  const text =
    typeof task === "string"
      ? task
      : `${task.goal || ""} ${task.objective || ""} ${task.role || ""}`;
  const lower = text.toLowerCase();

  if (
    /\b(vision|layout|ui audit|visual design|deepseek-v4.1-flash|deepseek v4)\b/i.test(
      lower,
    )
  )
    return "vision";
  if (
    /\b(architecture|schema|system design|spec|plan|brainstorm)\b/i.test(lower)
  )
    return "architecture";
  if (/\b(review|quality|audit|inspect|code review)\b/i.test(lower))
    return "review";
  if (/\b(flux|art|image|creative|copywriting)\b/i.test(lower))
    return "creative";
  if (/\b(format|lint|syntax|typecheck|mechanical)\b/i.test(lower))
    return "mechanical";
  return "standard";
}
...
export class SuperpowersWorkflowEngine {
  constructor(options = {}) {
    ...
    this.capabilityRegistry =
      options.capabilityRegistry || defaultCapabilityRegistry;
  ...
  async processRequest(userPrompt, options = {}) {
    ...
    try {
      ...
      workflow.completeStage(WORKFLOW_STAGES.INTENT_CLASSIFIED, {
        canonicalIntent,
      });
      ...
      workflow.startStage(WORKFLOW_STAGES.SKILLS_RESOLVED, {
        resolvedSkillIds: resolvedSkills.map((s) => s.id),
      });
      workflow.completeStage(WORKFLOW_STAGES.SKILLS_RESOLVED, {
      ...
      if (
        ...
        (resolvedSkills.length === 1 &&
          resolvedSkills[0].id === "using-superpowers")
      ) {
        workflow.startStage(WORKFLOW_STAGES.VERIFYING, { directAnswer: true });
        workflow.addVerificationRecord({
      ...
      if (activeSkillIds.has("brainstorming")) {
        workflow.startStage(
          ...
          { prompt: userPrompt },
          { agent: "ARCHITECT", skill: "brainstorming" },
        );
      ...
      if (activeSkillIds.has("writing-plans")) {
        workflow.startStage(
          ...
          { specification, prompt: userPrompt },
          { agent: "ARCHITECT", skill: "writing-plans" },
        );
        planTasks = await this.runPlanningPass(
          userPrompt,
          specification,
          options,
        );
        workflow.completeStage(WORKFLOW_STAGES.PLANNING, {
          taskCount: planTasks.length,
          tasks: planTasks,
        });
        workflow.startStage(WORKFLOW_STAGES.PLAN_READY, { tasks: planTasks });
        workflow.completeStage(WORKFLOW_STAGES.PLAN_READY, {
          tasksReady: true,
        });
      }
      ...
      workflow.startStage(
        WORKFLOW_STAGES.IMPLEMENTING,
        { taskCount: planTasks.length },
        { agent: "IMPLEMENTER", skill: "subagent-driven-development" },
      );
      const implementationOutputs = await this.runImplementationPass(
        userPrompt,
        planTasks,
        options,
      );
      ...
      if (activeSkillIds.has("requesting-code-review")) {
        workflow.startStage(
          WORKFLOW_STAGES.REVIEWING,
          { outputs: implementationOutputs },
          { agent: "REVIEWER", skill: "requesting-code-review" },
        );
        ...
        if (workflow.hasCriticalReviewFindings()) {
          workflow.startStage(
            WORKFLOW_STAGES.REPAIRING,
            { findings: workflow.reviewFindings },
            { agent: "REPAIR_AGENT", skill: "receiving-code-review" },
          );
          const repairOutputs = await this.runRepairPass(
            implementationOutputs,
            workflow.reviewFindings,
            options,
          );
      ...
      workflow.startStage(
        WORKFLOW_STAGES.VERIFYING,
        { userPrompt },
        { agent: "VERIFIER", skill: "verification-before-completion" },
      );
      let verificationRecord = await this.runVerificationPass(
        userPrompt,
        implementationOutputs,
        options,
      );
      ...
      const maxRepairAttempts =
        options.maxRepairAttempts ?? Number.MAX_SAFE_INTEGER;
      let lastRepairContent = null;
      while (
        (verificationRecord.exitCode !== 0 || verificationRecord.failed > 0) &&
        repairAttempts < maxRepairAttempts
      ) {
        repairAttempts++;
        workflow.startStage(
          WORKFLOW_STAGES.REPAIRING,
          { attempt: repairAttempts, failure: verificationRecord },
          { agent: "REPAIR_AGENT", skill: "auto-debugging" },
        );
        const repairOutputs = await this.runVerificationRepairPass(
          implementationOutputs,
          verificationRecord,
          options,
        );
        ...
        verificationRecord = await this.runVerificationPass(
          userPrompt,
          implementationOutputs,
          options,
        );
  ...
  async runBrainstormingPass(userPrompt, _options) {
    if (this.aiClient) {
      const response = await this.aiClient(
        `Create a structured design specification for: "${userPrompt}". Include core features, target layout, and key UI/technical requirements.`,
  ...
  async runPlanningPass(userPrompt, specification, _options) {
    if (this.aiClient) {
      const response = await this.aiClient(
        `Decompose the following request into a JSON array of implementation tasks: "${userPrompt}".
Specification: ${JSON.stringify(specification)}
Output ONLY a JSON array: [{"taskId":"task-1","role":"IMPLEMENTER","goal":"...","relevantFiles":["index.html"],"dependencies":[]}]`,
        { taskCategory: "architecture" },
      );
      try {
  ...
  async runImplementationPass(userPrompt, tasks, _options) {
    ...
    for (const task of tasks) {
      ...
      if (this.aiClient) {
        const response = await this.aiClient(
          `Execute subagent task brief:\n${JSON.stringify(brief, null, 2)}`,
          { taskCategory: category },
        );
        outputs[task.taskId] = response;
      } else {
        outputs[task.taskId] =
          `Executed subagent task ${task.taskId}: ${task.goal}`;
      }
    }
    return outputs;
  }
...
Return ONLY JSON: {"passed": true, "findings": []}`;
  ...
  async runRepairPass(outputs, findings, _options) {
    ...
    for (const f of findings) {
      if (f.severity === "critical") {
        repaired["repair-" + Date.now()] =
          `Applied fix for critical finding: ${f.message}`;
      }
    }
    return repaired;
  }
  ...
  async runVerificationRepairPass(outputs, verificationRecord, _options = {}) {
    ...
    if (this.aiClient) {
      ...
      const fixResult = await this.aiClient(repairPrompt, {
        taskCategory: "mechanical",
      });
      repaired["verify-repair-" + Date.now()] = fixResult;
    } else {
      repaired["verify-repair-" + Date.now()] =
        `Applied automated self-repair for verification errors: ${syntaxLogs || "Resolved test failures"}`;
    }
    return repaired;
  }
    ...
    for (const [taskId, content] of Object.entries(outputs || {})) {
      if (typeof content === "string") {
        ...
        if (Math.abs(openBraces - closeBraces) > 10) {
          syntaxErrors.push(
            `Task ${taskId}: Potential unbalanced braces (${openBraces} open vs ${closeBraces} close)`,
          );
        }
