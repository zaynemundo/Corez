...
// Workers AI text-to-image: runs FLUX/SDXL on the account's own Workers AI
// (no third-party key). Returns the image URL (R2 or data URL) or null on failure.
// options: width/height (256-1920), seed (integer), model (flux/sdxl-lightning alias).
export const WORKERS_AI_IMAGE_MODEL = "@cf/black-forest-labs/flux-2-klein-4b";
export const WORKERS_AI_SDXL_LIGHTNING_MODEL =
...
export async function generateWorkersAIImage(
  ...
  try {
    ...
    // Allow explicit model selection (sdxl-lightning, flux, etc.) — passed through to /api/image/cf
...
- MANDATORY CHAT BRIEF: your response MUST begin with a 1-2 sentence chat brief BEFORE the code block — game title in bold, what the player does to win/lose, and the controls. Example: "Here's **Neon Pong** — bounce the ball past your rival to score. Move with the Arrow keys, Space to launch." NEVER output a bare code block with no brief. NEVER write a long feature list, architecture summary, or "I built..." paragraph.
...
export const parseCommand = parseSlashCommand;
...
function detectRequestedImageModel(prompt, history) {
  const haystack = [
    String(prompt || ""),
  ...
  if (haystack.includes("dreamshaper"))
    return "@cf/lykon/dreamshaper-8-lcm";
  if (haystack.includes("sdxl-base") || haystack.includes("sdxl base"))
...
export async function runImageCommand(
  ...
  const requestedModel = detectRequestedImageModel(cleanPrompt, history);
...
export async function handleMixedQuestionImageRequest(
  ...
  const requestedModelMixed = detectRequestedImageModel(prompt, history);
  const [hostedResult, imageResult] = await Promise.allSettled([
    ...
    generateImage(
      ...
      signal,
      extractReferenceImage(history),
      requestedModelMixed ? { model: requestedModelMixed } : {},
    ),
  ]);
...
export async function generateAIResponse(
  ...
  if (isExplicitImageRequest(cleanPrompt)) {
    ...
    try {
      const requestedModelExplicit = detectRequestedImageModel(
        cleanPrompt,
        history,
      );
      const imageUrl = await generateImage(
        ...
        signal,
        extractReferenceImage(history),
        requestedModelExplicit ? { model: requestedModelExplicit } : {},
      );
      if (imageUrl) {
        return `![](${imageUrl})`;
      }
