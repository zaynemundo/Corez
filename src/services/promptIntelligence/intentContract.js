...
function enrichContactWithDefaults(contract, type) {
  ...
  if (typeDefaults.mustAchieve) {
    for (const item of typeDefaults.mustAchieve) {
      if (
        !mustAchieve.some((e) => e.toLowerCase().includes(item.toLowerCase()))
      ) {
  ...
  if (typeDefaults.mustNotInvent) {
    for (const item of typeDefaults.mustNotInvent) {
      if (
        !mustNotInvent.some((e) => e.toLowerCase().includes(item.toLowerCase()))
      ) {
...
const TYPE_DEFAULTS = {
  ...
  [INTENT_TYPES.BUG_FIX]: {
    ...
    mustNotInvent: ["new features", "unrelated refactors"],
  },
...
export function checkContractViolations(contract, enrichedPrompt) {
  if (!contract || !enrichedPrompt || typeof enrichedPrompt !== "string")
    return [];
  ...
  for (const forbidden of contract.mustNotInvent) {
    if (
      forbidden === "real customer testimonials" &&
      ...
      violations.push({
        rule: forbidden,
        reason:
          "Prompt introduces testimonial language that could suggest fabricated content",
      });
    }
    if (
      forbidden === "real pricing" &&
      /\b(\$\d+\.\d{2}|\$\d+|price.*\d+|costs.*\$|pricing.*plan)\b/i.test(
        lower,
      ) &&
      /\b(pricing|price|plan)\b/i.test(lower)
    ) {
      violations.push({
        rule: forbidden,
        reason: "Prompt suggests specific pricing amounts",
      });
    }
    if (
      forbidden === "real certifications" &&
      /\b(certified by|ISO|accredited|certification from)\b/i.test(lower)
    ) {
      violations.push({
        rule: forbidden,
        reason: "Prompt introduces certification claims",
      });
    }
    if (
      forbidden === "paid/subscription features unless requested" &&
      /\b(paid subscription|premium plan|subscription model|premium content)\b/i.test(
        lower,
      )
    ) {
      violations.push({
        rule: forbidden,
        reason: "Prompt adds unauthorized subscription/payment language",
      });
    }
    if (
      forbidden === "new features" &&
      /\b(new feature|additionally|bonus|extra.*feature)\b/i.test(lower)
    ) {
      violations.push({
        rule: forbidden,
        reason:
          "Prompt suggests scope expansion beyond the requested fix/change",
      });
    }
  }
