...
const INTENT_HANDLERS = [
  ...
  },
  {
    ...
    pattern:
      /\b(add|implement|build|integrate|create)\b.*\b(feature|module|component|page|api|endpoint|route|service|handler|auth|login|authentication|search|search bar|filter|pagination|upload|export|toggle|dark mode|light mode|theme|settings|sort|modal|form|button|sidebar|header|footer|dashboard|analytics)\b/i,
    signals: [
      "add",
      "implement",
      "integrate",
  ...
  },
  {
    ...
    pattern:
      /\b(fix|debug|repair|resolve|patch|bug|error|exception|crash|broken|not working|failing|issue)\b/i,
    signals: [
      "fix",
      "debug",
      "repair",
  ...
  },
  {
    ...
    pattern:
      /\b(refactor|rewrite|restructure|reorganise|reorganize|clean up|improve|optimise|optimize)\b.*\b(code|function|module|class|component|architecture|service|api|file|dir)\b|\b(refactoring)\b/i,
    signals: [
      "refactor",
      "rewrite",
      "restructure",
  ...
  },
  {
    ...
    pattern:
      /\b(how|what|why|explain|tell me|show me|help)\b.*\b(code|js|javascript|react|python|css|html|function|component|api|library|framework|syntax|pattern)\b|\b(explain|understand|learn)\b.*\b(code|programming)\b/i,
    signals: [
      "how",
      "what",
      "why",
  ...
  },
  {
    ...
    pattern:
      /\b(research|search for|search the web|look up|investigate|analyse|analyze|compare|definition of|what is|what are|how does|how do|tell me about|difference between|find out)\b/i,
    signals: [
      "research",
      "search for",
      "look up",
  ...
  },
  {
    ...
    pattern:
      /\b(design|redesign|layout|wireframe|mockup|ux|ui|style|theme|color scheme|typography)\b.*\b(for|of|page|app)\b|\b(design|redesign)\b.*\b(component|page|screen)\b/i,
    signals: [
      "design",
      "redesign",
      "layout",
  ...
  },
  {
    ...
    pattern:
      /\b(generate|create|make|draw|render|produce)\b.*\b(image|picture|photo|artwork|illustration|graphic|icon|sprite|logo|banner|poster)\b|\b(image generation)\b/i,
    signals: [
      "image",
      "picture",
      "photo",
  ...
  },
  {
    ...
    pattern:
      /\b(change|update|rename|move|delete|remove|set|color|colour|font|size|width|height|margin|padding)\b.*\b(to|the|this|file|component)\b|\b(tweak|adjust|switch|swap)\b/i,
    signals: [
      "change",
      "update",
      "rename",
...
function extractDomain(lower) {
  const patterns = {
    "e-commerce":
      /\b(shop|store|ecommerce|e-commerce|product|retail|selling|buy|cart|checkout)\b/i,
    portfolio: /\b(portfolio|showcase|gallery|personal|CV|resume)\b/i,
    furniture: /chair|furniture|desk|table|sofa/i,
    healthcare:
      /\b(health|medical|patient|doctor|clinic|fitness|wellness|therapy)\b/i,
    education:
      /\b(education|school|university|course|learn|teach|academy|training)\b/i,
...
const BLOCKING_QUESTIONS = [
  {
    ...
    message:
      "Unclear what file or code to modify when multiple implementations exist",
    blocking: true,
  },
];
...
const OPTIONAL_QUESTIONS = [
  {
    keyword: /\b(colou?r|color scheme|theme|palette)\b/i,
    message: "Preferred colour scheme was not specified",
    blocking: false,
  },
  {
    keyword: /\b(font|typography)\b/i,
    message: "Font preference was not specified",
    blocking: false,
  },
  {
    keyword: /\b(logo|brand|identity|name)\b/i,
    message: "Brand identity was not provided",
    blocking: false,
  },
  {
    keyword: /\b(pricing|price)\b/i,
    message: "Pricing information was not provided",
    blocking: false,
  },
  {
    keyword: /\b(hosting|deploy|domain|url)\b/i,
    message: "Hosting/deployment preference was not specified",
    blocking: false,
  },
];
...
function detectIsExistingProject(lower, primaryType) {
  ...
  const existingProjectSignals =
    /\b(repo|repository|file|files|existing|codebase|src\/|component|function|line|in this project|my app|our app|current code|update|modify|patch|refactor|fix|debug)\b/i;
...
function detectOutputFormat(lower, primaryType) {
  ...
  if (
    ...
    /\b(react|jsx|component|app|dashboard|widget)\b/i.test(lower)
  ) {
    return "jsx";
  }
...
function isCVRewriteRequest(lower) {
  ...
  const hasRewriteSignal =
    /\b(less description|less text|shorten|short version|condense|condensed|concise|make.*concise|make.*shorter|rewrite|rephrase|reduce|brief|summarize|summary|bullet|one line)\b/i.test(
      lower,
    ) || /\bgive me less/i.test(lower);
  const hasPastedCVContent =
    lower.includes("created proposals") ||
    lower.includes("developed websites") ||
    (lower.includes("social media") && lower.includes("analytics"));
...
function hasExplicitWebCreationInInstruction(instructionLower) {
  return /\b(build|make|create|generate|develop|design|launch|ship)\b.*\b(website|site|landing page|webpage|web app|web application|homepage|portfolio site)\b|\b(website|site|landing page|portfolio)\b.*\b(build|make|create|generate|design|develop)\b/i.test(
    instructionLower,
  );
}
...
export function classifyIntent(prompt) {
  ...
  if (isCVRewriteRequest(lower)) {
    ...
    const hasWebInInstruction =
      hasExplicitWebCreationInInstruction(instructionLower);
  ...
  for (const handler of INTENT_HANDLERS) {
    const matched =
      handler.pattern.test(prompt) ||
      handler.signals.some((s) => lower.includes(s));
    if (matched) {
  ...
  const pastHistoryMarkers = [
    "created proposals",
    "collaborated with the design",
    "monitored social media",
    "developed websites using",
    "managed advertising",
    "maintained close contact with clients",
    "scheduled social media posts",
  ];
  const resumeSignalCount = pastHistoryMarkers.filter((m) =>
    lower.includes(m),
  ).length;
  const looksLikeResumeDump = resumeSignalCount >= 2;

  if (
    !hasExplicitWebCreationInInstruction(instructionLowerFallback) &&
    /\b(give me less|less description|condense|rewrite|rephrase|make.*concise|make.*shorter|short version|concise version)\b/i.test(
      lower,
    )
  ) {
    const filtered = candidateMatches.filter(
      (c) =>
        c.handler.type !== INTENT_TYPES.WEBSITE_CREATION &&
        c.handler.type !== INTENT_TYPES.GAME_CREATION,
    );
    if (filtered.length > 0 && filtered[0].confidence >= 0.15) {
      ...
      candidateMatches.some(
        (c) => c.handler.type === INTENT_TYPES.WEBSITE_CREATION,
      )
    ) {
  ...
  if (
    ...
    const filteredResume = candidateMatches.filter(
      (c) => c.handler.type !== INTENT_TYPES.WEBSITE_CREATION,
    );
  ...
  const hasRequestVerb =
    /\b(need|want|looking for|hire|seeking|require|build|make|create|generate|design|launch|ship|make me|create me|build me)\b/i.test(
      instructionLowerFallback,
    );
  const pastResumeVerbs = (
    lower.match(
      /\b(developed|created|collaborated|monitored|managed|maintained|scheduled|designed)\b/gi,
    ) || []
  ).length;
  if (
    !hasRequestVerb &&
    pastResumeVerbs >= 1 &&
    pastResumeVerbs >=
      (lower.match(/\b(website|site|landing page)\b/gi) || []).length
  ) {
    // Check if top candidate is website_creation via signals only
    const webHandler = INTENT_HANDLERS.find(
      (h) => h.type === INTENT_TYPES.WEBSITE_CREATION,
    );
    ...
    if (
      ...
      const filteredPast = candidateMatches.filter(
        (c) => c.handler.type !== INTENT_TYPES.WEBSITE_CREATION,
      );
      if (filteredPast.length > 0) {
        ...
        if (
          handler &&
          /\b(rewrite|condense|summarize|less description|shorten)\b/i.test(
            lower,
          )
        ) {
  ...
  if (candidateMatches.length === 0 || candidateMatches[0].confidence < 0.15) {
    const isExisting = detectIsExistingProject(
      lower,
...
export function extractRequirements(prompt, intent) {
  ...
  if (
    /\b(i18n|internationalization|multi-language|translation)\b/i.test(prompt)
  ) {
  ...
  const actionMatch = prompt.match(
    /\b(build|make|create|generate|design|code|fix|add|implement|change|update)\b/i,
  );
  if (actionMatch) {
  ...
  if (
    !/\b(responsive|mobile|desktop)\b/i.test(prompt) &&
    intent.type !== INTENT_TYPES.SIMPLE_EDIT &&
    intent.type !== INTENT_TYPES.CODE_QUESTION
  ) {
  ...
  if (
    (intent.type === INTENT_TYPES.WEBSITE_CREATION ||
      intent.type === INTENT_TYPES.GAME_CREATION) &&
  ...
  forbidden.push(
    "inventing real testimonials or reviews",
    "inventing factual claims about real companies",
    "inventing pricing for real products",
  );
...
export function detectMissingInformation(prompt, intent) {
  ...
  for (const q of BLOCKING_QUESTIONS) {
    if (
      q.keyword.test(lower) &&
      !/\.[a-z0-9]{1,8}\b|\/[a-z0-9_.-]+/i.test(lower)
    ) {
      missing.push({ ...q });
    }
  }
  ...
  if (
    ...
    if (
      !/\b(page|section|component|hero|footer|navbar|specific)\b/i.test(lower)
    ) {
      missing.push({
        message: "Specific pages or sections were not listed",
        blocking: false,
      });
    }
  }
...
export function classifyComplexity(prompt, intent) {
  ...
  if (
    ...
    return /\b(complex|architecture|major|overhaul)\b/i.test(lower)
  ...
  if (type === INTENT_TYPES.FEATURE_IMPLEMENTATION) {
    if (
      /\b(auth|authentication|payment|subscription|real-time|websocket|database|migration)\b/i.test(
        lower,
      )
    )
  ...
  if (type === INTENT_TYPES.WEBSITE_CREATION) {
    if (
      /\b(epic|enterprise-scale|massive|large-scale|full platform|complete platform|AAA|metaverse)\b/i.test(
        lower,
      )
    )
      return COMPLEXITY_LEVELS.EPIC;
    if (
      /\b(SaaS|enterprise|production|full-stack|complex|complete|full)\b/i.test(
        lower,
      )
    )
      return COMPLEXITY_LEVELS.HIGH;
    if (/\b(landing|simple|single page|portfolio)\b/i.test(lower))
      return COMPLEXITY_LEVELS.LOW;
    return COMPLEXITY_LEVELS.MEDIUM;
  }
  ...
  if (type === INTENT_TYPES.GAME_CREATION) {
    if (
      /\b(epic|massive|large-scale|full platform|AAA|metaverse|persistent world|open world)\b/i.test(
        lower,
      )
    )
      return COMPLEXITY_LEVELS.EPIC;
    if (
      /\b(multiplayer|mmo|rpg|role playing|complex|large|production)\b/i.test(
        lower,
      )
    )
      return COMPLEXITY_LEVELS.HIGH;
    if (/\b(snake|pong|tictactoe|quiz|memory|simple|basic)\b/i.test(lower))
      return COMPLEXITY_LEVELS.LOW;
    return COMPLEXITY_LEVELS.MEDIUM;
  }
