...
export function guardIntent(rawPrompt, contract, enrichedPrompt, intent) {
  ...
  const forbiddenTerms = [
    ...
    {
      term: /CMS|content management system/i,
      message: "CMS was never requested",
    },
    { term: /analytics/i, message: "Analytics was never requested" },
    {
      term: /3D animation|three\.js/i,
      message: "3D animations were never requested",
    },
    {
      term: /artificial intelligence|AI assistant|machine learning/i,
      message: "AI/ML integration was never requested",
    },
    {
      term: /database|backend\s*server|API endpoint|sql|mongo|postgres/i,
      message: "Backend/database was never requested",
    },
    {
      term: /authentication|login|sign up|user account/i,
      message: "Authentication was never requested",
    },
    {
      term: /payment|checkout|billing|subscription|stripe/i,
      message: "Payment/billing was never requested",
    },
    {
      term: /cloud formation|AWS|Azure|GCP|deploy|CI\/CD|Docker/i,
      message: "Cloud infrastructure was never requested",
    },
  ];

  const isSimpleRequest =
    /\b(simple|basic|quick|minimal|just|only|small)\b/i.test(rawLower);

  for (const { term, message } of forbiddenTerms) {
    if (
      term.test(enrichedLower) &&
      !term.test(rawLower) &&
      !isInProhibitionContext(enrichedPrompt, term)
    ) {
  ...
  if (contract && Array.isArray(contract.mustNotInvent)) {
    for (const rule of contract.mustNotInvent) {
      ...
      if (
        ruleLower.includes("testimonial") &&
        /\b(testimonials?|what our customer|client says|happy customer)\b/i.test(
          enrichedLower,
        ) &&
        !isInProhibitionContext(enrichedPrompt, /testimonials?/i)
      ) {
        violations.push({
          severity: "critical",
          message: "Prompt introduces potentially fabricated testimonials",
        });
      }
      if (
        ruleLower.includes("pricing") &&
        /\b(price.*\$|pricing.*plan|premium.*plan)\b/i.test(enrichedLower) &&
        !isInProhibitionContext(enrichedPrompt, /pricing/i)
      ) {
        violations.push({
          severity: "important",
          message:
            "Prompt introduces pricing information that was not supplied",
        });
      }
    }
  }
  ...
  if (isSimpleRequest) {
    ...
    if (complexSignalCount >= 3) {
      violations.push({
        severity: "critical",
        message:
          "Simple request is being over-complicated with unnecessary enterprise/complexity language",
      });
    }
  }
  ...
  const importantCount = violations.filter(
  ...
  return {
    intentDrift,
    reason: violations
      .filter((v) => v.severity === "critical" || v.severity === "important")
      .map((v) => v.message)
      .join("; "),
    violations,
  };
...
export function deEscalate(enrichedPrompt, guardResult, _intent) {
  ...
  for (const v of allViolations) {
    ...
    if (v.message.includes("Authentication"))
      removals.push(/\bauthentication\b/gi);
  ...
  if (!cleaned || cleaned.length < 10) {
    ...
    cleaned = firstSentence
      .replace(/WebGL|CMS|analytics|authentication|three\.js/gi, "")
      .replace(/\s{2,}/g, " ")
      .trim();
...
function isInProhibitionContext(prompt, forbiddenTerm) {
  ...
  while ((match = regex.exec(prompt)) !== null) {
    ...
    const contextBefore = prompt
      .slice(Math.max(0, pos - 120), pos)
      .toLowerCase();
    if (
      /\b(do not|must not|cannot|don't|shouldn't|never|avoid|forbid|prohibited|not include)\b/.test(
        contextBefore,
      )
    ) {
      return true;
