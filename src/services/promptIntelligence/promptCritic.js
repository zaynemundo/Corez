...
export function critiquePrompt(
  ...
  const preservationScore = scoreIntentPreservation(
    rawPrompt,
    enrichedPrompt,
    intent,
  );
  dimensions.push({
    name: "intentPreservation",
  ...
  for (const dim of dimensions) {
    if (dim.score < dim.max * 0.5) {
      result.issues.push(
        `${dim.name} is weak (${Math.round((dim.score / dim.max) * 100)}%)`,
      );
  ...
  if (rawWords >= 3 && enrichedWords > rawWords * 8) {
    result.issues.push(
      "Enriched prompt is significantly longer than the original — possible over-enrichment",
    );
    result.recommendedImprovements.push(
      "Reduce prompt size by removing redundant elaboration",
    );
  }

  if (
    !/test|validate|verify/i.test(enrichedPrompt) &&
    enrichedPrompt.length > 100
  ) {
    result.issues.push("No testing or validation requirements specified");
    result.recommendedImprovements.push(
      "Add explicit test and validation requirements",
    );
  }
...
function scoreIntentPreservation(raw, enriched, intent) {
  ...
  if (
    intent &&
    intent.domain &&
    !enrichedLower.includes(intent.domain.toLowerCase())
  ) {
    score *= 0.8;
  }
...
function scoreClarity(enriched) {
  ...
  if (/do something|somehow|maybe|perhaps|possibly/i.test(enriched))
    score *= 0.8;
...
function scoreScopeControl(raw, enriched, requirements) {
  ...
  for (const f of forbidden) {
    ...
    if (
      fLower.includes("pricing") &&
      /pricing.*features|paid|subscription|premium/i.test(enrichedLower)
    ) {
      score *= 0.6;
    }
  }
  ...
  if (
    enriched.replace(/\s+/g, " ").length >
    raw.replace(/\s+/g, " ").length * 5
  ) {
    score *= 0.7;
  }
...
function scoreContextUsage(enriched) {
  ...
  if (
    /\b(React|Next\.js|Vue|Svelte|Angular|Tailwind|Sass|styled-components|TypeScript|JavaScript)\b/i.test(
      enriched,
    )
  ) {
    return 1.0;
