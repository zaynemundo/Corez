...
export function validateGameManifest(data) {
  ...
  if (!data.gameSpec || typeof data.gameSpec !== "object") {
    ...
    if (typeof title !== "string" || !title.trim())
      errors.push("gameSpec.title must be a non-empty string.");
    if (typeof genre !== "string" || !genre.trim())
      errors.push("gameSpec.genre must be a non-empty string.");
    if (!Array.isArray(mechanics))
      errors.push("gameSpec.mechanics must be an array.");
    if (!controls || typeof controls !== "object")
      errors.push("gameSpec.controls must be an object.");
    if (!Array.isArray(entities))
      errors.push("gameSpec.entities must be an array.");
    if (typeof winCondition !== "string")
      errors.push("gameSpec.winCondition must be a string.");
    if (typeof loseCondition !== "string")
      errors.push("gameSpec.loseCondition must be a string.");
    if (!Array.isArray(difficultyCurve))
      errors.push("gameSpec.difficultyCurve must be an array.");
  }
  ...
  if (!data.artDirection || typeof data.artDirection !== "object") {
    ...
    if (typeof style !== "string" || !style.trim())
      errors.push("artDirection.style must be a non-empty string.");
    if (!Array.isArray(palette))
      errors.push("artDirection.palette must be an array.");
    if (typeof camera !== "string")
      errors.push("artDirection.camera must be a string.");
    if (!Array.isArray(renderingRules))
      errors.push("artDirection.renderingRules must be an array.");
  }
  ...
  if (!data.assetManifest || typeof data.assetManifest !== "object") {
    ...
    data.assetManifest.assets.forEach((asset, idx) => {
      ...
      if (!validTypes.has(asset.type)) {
        errors.push(
          `${prefix}.type "${asset.type}" is invalid. Expected one of: ${Array.from(validTypes).join(", ")}.`,
        );
      }
