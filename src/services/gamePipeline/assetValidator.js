...
export function validateAsset(assetDef, assetData) {
  ...
  if (assetData.sizeBytes && assetData.sizeBytes > MAX_SIZE) {
    errors.push(
      `Asset file size (${assetData.sizeBytes} bytes) exceeds maximum threshold of 5MB.`,
    );
  }
  ...
  if (assetData.width && assetData.height) {
    ...
    if (ratioDiff > 0.25) {
      warnings.push(
        `Asset aspect ratio (${actualRatio.toFixed(2)}) deviates from target ratio (${expectedRatio.toFixed(2)}).`,
      );
    }
  }
  ...
  if (assetDef.transparent && assetData.hasAlpha === false) {
    errors.push(
      `Asset "${assetDef.id}" requires a transparent background, but non-transparent image was provided.`,
    );
  }
