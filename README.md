...
When `OPENCODE_GO_API_KEY` (or `OPENCODE_API_KEY`) is configured, `/api/ai` routes exclusively through the **OpenCode Go API** (it serves the latest DeepSeek V4.1 Flash builds). Chat has no DeepSeek or OpenRouter fallback — `/api/ai` uses `OPENCODE_GO_API_KEY` only:

1. **OpenCode Go** — `OPENCODE_GO_API_KEY` / `OPENCODE_API_KEY`, endpoint `OPENCODE_ENDPOINT` (default `https://opencode.ai/zen/go/v1/chat/completions`), model `OPENCODE_MODEL` (default `deepseek-v4.1-flash`).

All chat, coding, app & swarm requests use `deepseek-v4.1-flash` unless overridden. The model list is server-controlled: client-supplied `body.model` is never trusted. The chat provider can be disabled with `OPENCODE_GO_DISABLED` (any truthy value). The provider key never leaves the worker, and the content returned to users is provider-neutral. Image generation (`/api/image`) still uses `OPENROUTER_API_KEY` separately when configured.
...
The canvas preview's **Publish** button shares the current creation as a short public link (`corez.pro/asyag23-123`). Publishing is available on every plan: Free publications carry a small "Made with Corez" badge injected server-side, while Standard and Premium publish badge-free and can customize the slug. The worker stores the formatted preview document in R2 under `publish/<slug>.json` and serves it at the bare root path to anyone, sandboxed with the same CSP as the in-app preview. Publishing again updates the same link: identical content, a revised version from the same chat session, and multi-page sub-pages all republish under the existing slug rather than creating duplicates.
