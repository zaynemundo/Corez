...
export async function verifyPassword(password, stored) {
  ...
  for (let i = 0; i < a.length; i++)
    diff |= a.charCodeAt(i) ^ hashB64.charCodeAt(i);
  return diff === 0;
}
...
export async function findUserByEmail(env, email) {
  ...
  return await env.DB.prepare("SELECT * FROM users WHERE lower(email)=lower(?)")
    .bind(email)
    .first();
}
...
export async function handleAuth(request, env) {
  ...
  if (path === "/api/auth/me" && request.method === "GET") {
    ...
    try {
      ...
      if (u && typeof u.plan === "string" && u.plan)
  ...
  if (path === "/api/auth/signup" && request.method === "POST") {
    ...
    if (!env?.AUTH_SECRET)
      return jsonResponse(500, { error: "AUTH_SECRET not configured" });
    ...
    try {
      await env.DB.prepare(
        "INSERT INTO users (id, email, password_hash, provider, created_at, plan) VALUES (?,?,?,?,?,?)",
      )
        .bind(id, email, hash, "local", Date.now(), plan)
        .run();
    } catch {
      await env.DB.prepare(
        "INSERT INTO users (id, email, password_hash, provider, created_at) VALUES (?,?,?,?,?)",
      )
        .bind(id, email, hash, "local", Date.now())
        .run();
      try {
    ...
    return new Response(
      JSON.stringify({ ok: true, user: { id, email, plan } }),
      {
        status: 200,
        headers: {
          "Content-Type": "application/json",
  ...
  if (path === "/api/auth/login" && request.method === "POST") {
    ...
    const plan =
      (user.plan && String(user.plan).trim().toLowerCase()) || "free";
    const token = await createJWT(
      {
        uid: user.id,
        email: user.email,
  ...
  if (path === "/api/auth/google" && request.method === "GET") {
    ...
    if (!clientId)
      return jsonResponse(500, { error: "Google OAuth not configured" });
  ...
  if (path === "/api/auth/google/callback" && request.method === "GET") {
    ...
    try {
      ...
      return jsonResponse(502, {
        error: "Google token exchange failed: " + safeErrorDetail(e),
      });
    }

    const idToken = tokenData?.id_token;
    if (!idToken)
      return jsonResponse(502, { error: "No id_token from Google" });

    let googlePayload;
    try {
      const parts = idToken.split(".");
      googlePayload = JSON.parse(
        new TextDecoder().decode(b64urlDecode(parts[1])),
      );
    } catch {
      return jsonResponse(502, { error: "Invalid id_token" });
    }

    const email = String(googlePayload?.email || "").toLowerCase();
    const googleId = String(googlePayload?.sub || "");
    if (!validEmail(email))
      return jsonResponse(400, { error: "Google account has no email" });
    if (!env?.DB)
      return jsonResponse(500, { error: "Auth database not configured" });
    ...
    if (!user) {
      ...
      try {
        await env.DB.prepare(
          "INSERT INTO users (id, email, password_hash, provider, google_id, created_at, plan) VALUES (?,?,?,?,?,?,?)",
        )
          .bind(id, email, null, "google", googleId, Date.now(), "free")
          .run();
      } catch {
        await env.DB.prepare(
          "INSERT INTO users (id, email, password_hash, provider, google_id, created_at) VALUES (?,?,?,?,?,?)",
        )
          .bind(id, email, null, "google", googleId, Date.now())
          .run();
      }
  ...
  if (path === "/api/auth/forgot" && request.method === "POST") {
    ...
    if (retryAfter !== null) {
      return jsonResponse(
        429,
        { error: "Too many reset attempts. Please wait." },
        { "Retry-After": String(retryAfter) },
      );
    }
    let body;
    ...
    if (user && user.password_hash) {
      ...
      try {
        await env.DB.prepare(
          ...
          .bind(id, email, token, expiresAt, Date.now())
          .run();
...
<div style="background:#f4f4f5;padding:32px 16px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;">
  ...
  if (path === "/api/auth/reset" && request.method === "POST") {
    ...
    if (Number(row.used) === 1)
      return jsonResponse(400, { error: "Reset token already used" });
    if (Date.now() > Number(row.expires_at))
      return jsonResponse(400, { error: "Reset token expired" });
