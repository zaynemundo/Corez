...
const MEDIA_DISAMBIG_PATTERN =
  /\((?:song|single|album|ep|band|film|movie|video game|television series|tv series|novel|book|musical|opera|poem|magazine|song cycle|extended play)\)\s*$/i;
...
function normalizeResult(title, url, snippet, source) {
  ...
  const cleanSnippet =
    typeof snippet === "string" ? snippet.trim().slice(0, 500) : "";
...
function parseCurrencyConversion(query) {
  const match = String(query || "").match(
    /\b(\d[\d,]*(?:\.\d+)?)\s+([a-z]{3})\s+(?:to|in|into)\s+([a-z]{3})\b/i,
  );
...
async function searchCurrencyRate(query, fetchImpl) {
  ...
  const fallbackUrl = new URL(
    `${OPEN_EXCHANGE_RATE_ENDPOINT}${conversion.from}`,
  );
  const fallbackResponse = await fetchImpl(fallbackUrl, providerFetchOptions());
  if (!fallbackResponse.ok)
    throw new Error(`ExchangeRate-API HTTP ${fallbackResponse.status}`);
...
async function searchDuckDuckGo(query, fetchImpl) {
  ...
  while ((match = linkPattern.exec(html)) !== null) {
    links.push({
      href: match[1],
      title: decodeHtmlText(stripHtmlTags(match[2])).trim(),
    });
...
async function rankWithEmbeddingsProvider(query, results, provider) {
  ...
  if (!Array.isArray(queryVector) || docVectors.size < results.length)
    return null;
  ...
  return {
    ...
    results: [...results]
      .map((result, index) => ({
        result,
        score: cosineSimilarity(queryVector, docVectors.get(index)),
      }))
      .sort((a, b) => b.score - a.score)
      .map((entry) => entry.result),
  };
...
async function rankSearchResults(query, results, env) {
  ...
  const workersAIEmbedded = await rankWithEmbeddingsWorkersAI(
    query,
    results,
    env,
  );
    ...
    scored
      .filter(
        (entry) =>
          ...
          (Number.isFinite(entry?.index) || Number.isFinite(entry?.id)),
      )
      .map((entry) => [
        Number.isFinite(entry.index) ? entry.index : entry.id,
        entry.score,
      ]),
  );
  if (scoreByIndex.size < results.length) return null;
...
async function rankWithEmbeddingsWorkersAI(query, results, env) {
  ...
  if (!Array.isArray(queryVector) || docVectors.length < results.length)
    return null;
  ...
  await Promise.allSettled(
    unique.map(async (title) => {
      ...
      for (const page of Object.values(pages)) {
        ...
        if (extract)
          extracts.set(page.title, extract.slice(0, MAX_EXTRACT_CHARS));
      }
    }),
  );
  return extracts;
...
async function searchExa(query, env, fetchImpl, detail = false) {
  ...
  for (const hit of hits) {
    ...
    const highlight =
      Array.isArray(hit.highlights) && hit.highlights.length > 0
...
async function searchWikipedia(query, fetchImpl) {
  ...
  const appendHit = (hit, withSnippet) => {
    ...
    const snippet =
      withSnippet && typeof hit.snippet === "string"
  ...
  return promoteTitleMatches(
    filterMediaNoise(results, searchTerm),
    searchTerm,
  ).slice(0, MAX_RESULTS);
}
...
export async function handleSearch(request, env) {
  ...
  if (env?.EXA_API_KEY) {
    providers.push(async () =>
      searchExa(searchTerm, env, fetchImpl, body?.detail === true),
    );
  }
  providers.push(
  ...
  for (
    let index = 0;
    index < maxLen && merged.length < MAX_RESULTS;
    index += 1
  ) {
