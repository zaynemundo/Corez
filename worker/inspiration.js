...
const CATEGORIES = Object.freeze([
  ...
  },
  {
    key: "agency",
    url: `${BASE}/agency/`,
    keywords: ["agency", "studio", "company", "consulting", "services", "firm"],
  },
  {
    key: "gaming",
    url: `${BASE}/gaming/`,
...
export async function fetchSiteDetails(slug, fetchImpl = fetch) {
  ...
  try {
    ...
    const liveMatch = html.match(
      /<a[^>]+href="(https?:\/\/(?!www\.awwwards\.com)[^"]+)"[^>]*>(?:[^<]*(?:Visit|Live|View|Launch)[^<]*)/i,
    );
    ...
    const videoMatches = [
      ...html.matchAll(
        /(?:src|data-src)="([^"]*\/awards\/element\/[^"]+\.mp4)"/gi,
      ),
    ].map((m) => m[1].trim());
    const videoUrls = [...new Set(videoMatches)].slice(0, 3);

    const tagMatches = [
      ...html.matchAll(/href="\/websites\/([a-z0-9-]+)\/"[^>]*>([^<]+)<\/a>/gi),
    ]
      .map((m) => m[2].trim())
      .filter(
        (t) =>
...
export async function fetchAwwwardsInspiration(query, fetchImpl = fetch) {
  ...
  const slugs = [
    ...new Set(
      [...html.matchAll(/\/sites\/([a-z0-9-]{3,60})/g)].map((m) => m[1]),
    ),
  ];
  ...
  const sites = candidateSlugs.map((slug, idx) => {
    const detail =
      siteDetailsResults[idx]?.status === "fulfilled"
        ? siteDetailsResults[idx].value
        : null;
    const siteObj = {
    ...
    if (detail) {
      ...
      if (detail.videoUrls && detail.videoUrls.length > 0)
        siteObj.videoUrls = detail.videoUrls;
...
export async function handleInspiration(request, env) {
  ...
  if (!query || query.length > 300) {
    return jsonResponse(400, {
      error: "Query must be a non-empty string up to 300 characters.",
    });
  }
