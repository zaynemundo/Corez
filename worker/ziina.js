...
function validateUrls({ success_url, cancel_url, failure_url }) {
  ...
  if (
    ...
    if (!isValidHttpsUrl(success_url))
      errors.push("success_url must be a valid https URL");
  }
  if (
    cancel_url !== undefined &&
    cancel_url !== null &&
    String(cancel_url).trim() !== ""
  ) {
    if (!isValidHttpsUrl(cancel_url))
      errors.push("cancel_url must be a valid https URL");
  }
  if (
    failure_url !== undefined &&
    failure_url !== null &&
    String(failure_url).trim() !== ""
  ) {
    if (!isValidHttpsUrl(failure_url))
      errors.push("failure_url must be a valid https URL");
  }
  return errors;
}
...
export async function handleZiina(request, env) {
  ...
  const isCreate =
    (pathname === "/api/ziina/payment_intent" ||
      pathname === "/api/ziina/payment-intent" ||
      pathname === "/api/payments/ziina/create-intent" ||
      pathname === "/api/payments/ziina/create" ||
      pathname === "/api/payments/ziina/payment_intent") &&
    request.method === "POST";
  const isGetById =
    pathname.startsWith("/api/ziina/payment_intent/") ||
    pathname.startsWith("/api/ziina/payment-intent/");
  ...
  if (!apiKey) {
    return jsonResponse(500, {
      error: "Ziina payment gateway not configured: ZIINA_API_KEY missing",
    });
  }
  ...
  if (request.method === "GET" && (isGetById || isGetByIdAlt)) {
    let id = null;
    if (pathname.startsWith("/api/ziina/payment_intent/"))
      id = decodeURIComponent(
        pathname.slice("/api/ziina/payment_intent/".length),
      );
    else if (pathname.startsWith("/api/ziina/payment-intent/"))
      id = decodeURIComponent(
        pathname.slice("/api/ziina/payment-intent/".length),
      );
    ...
    try {
      const ziinaRes = await fetch(
        `${ZIINA_BASE}/payment_intent/${encodeURIComponent(id)}`,
        {
          method: "GET",
          headers: {
      ...
      if (!ziinaRes.ok) {
        return jsonResponse(ziinaRes.status, {
          error: data?.message || data?.error || "Ziina fetch failed",
          detail: data,
          ziinaStatus: ziinaRes.status,
        });
      }
      return jsonResponse(200, data);
    } catch (err) {
      return jsonResponse(502, {
        error: "Failed to fetch Ziina payment intent",
  ...
  if (isCreate) {
    ...
    if (planMeta) {
      ...
      amount = body.amount;
      if (amount === undefined && body.amount_fils !== undefined)
        amount = body.amount_fils;
      if (amount === undefined && body.amountInFils !== undefined)
        amount = body.amountInFils;
      if (amount === undefined && body.amount_aed !== undefined)
        amount = Number(body.amount_aed) * 100;
      if (amount === undefined && body.amountAed !== undefined)
        amount = Number(body.amountAed) * 100;
      if (amount === undefined && body.amountAED !== undefined)
        amount = Number(body.amountAED) * 100;
    ...
    if (!Number.isFinite(amount)) {
      return jsonResponse(400, {
        error:
          'amount or plan is required. Use plan: "free" (0 AED), "standard" (1836 fils = 18.36 AED) or "premium" (2754 fils = 27.54 AED), or amount in fils (100 AED = 10000 fils). Minimum 200 fils (2 AED).',
      });
    }
    ...
    if (amount < MIN_FILS) {
      return jsonResponse(400, {
        error: `amount must be at least ${MIN_FILS} fils (2 AED). Got ${amount} fils. Remember: 100 AED = 10000 fils.`,
      });
    }
    if (amount > 10000000) {
      // 100,000 AED cap sanity
      return jsonResponse(400, {
        error: "amount exceeds maximum (10000000 fils = 100000 AED)",
      });
    }

    const currency_code = String(
      body.currency_code || body.currencyCode || body.currency || "AED",
    )
      .trim()
      .toUpperCase();
    if (!/^[A-Z]{3}$/.test(currency_code)) {
      return jsonResponse(400, {
        error: "currency_code must be a 3-letter ISO code, e.g. AED",
      });
    }
    ...
    if (body.operation_id || body.operationId)
      ziinaPayload.operation_id = String(
