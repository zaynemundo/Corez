...
export async function handleRerank(request, env) {
  ...
  if (!hasWorkersAIBinding(env)) {
    return jsonResponse(503, {
      error:
        "Workers AI reranking is unavailable: no AI binding is configured on this deployment.",
    });
  }

  const topK =
    body.top_k === undefined || body.top_k === null
      ? undefined
      : Math.min(
          MAX_RERANK_CONTEXTS,
          Math.max(1, Math.round(Number(body.top_k))),
        );
  ...
  try {
    ...
    return jsonResponse(502, {
      error: "Reranking failed: the Workers AI provider returned an error.",
    });
  }
  ...
  const results = scored
    ...
    .map((entry) => ({
      index: Number.isFinite(entry.index) ? entry.index : entry.id,
      score: entry.score,
    }));
  if (results.length === 0) {
    return jsonResponse(502, {
      error: "Reranking failed: the Workers AI provider returned no scores.",
    });
  }
...
export async function handleEmbed(request, env) {
  ...
  if (!hasWorkersAIBinding(env)) {
    return jsonResponse(503, {
      error:
        "Workers AI embeddings are unavailable: no AI binding is configured on this deployment.",
    });
  }
  ...
  try {
    ...
    return jsonResponse(502, {
      error: "Embedding failed: the Workers AI provider returned an error.",
    });
  }
  ...
  const data = vectors
    ...
    .map((vector) =>
      vector.map((value) => (Number.isFinite(value) ? value : 0)),
    );
  if (
    data.length !== texts.length ||
    data.some((vector) => vector.length === 0)
  ) {
    return jsonResponse(502, {
      error: "Embedding failed: the Workers AI provider returned no vectors.",
    });
  }
