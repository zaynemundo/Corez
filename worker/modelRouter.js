...
const VISUAL_KEYWORDS_PATTERN =
  /\b(ui|ux|css|styling|styles|layout|design|color|theme|dark mode|light mode|glassmorphism|aesthetic|visual|svg|canvas particle|animation|look|banner|poster|wireframe|mockup|button style|gradient|typography)\b/i;
 ...
 * @returns {boolean}
 */
export function isVisualRequest({
  prompt = "",
  intent = null,
  fineIntent = null,
  skills = [],
  ...
  if (Array.isArray(messages)) {
    for (const msg of messages) {
      if (
        ...
        msg.attachments.some(
          (a) =>
            ...
            (typeof a?.thumb === "string" && a.thumb.startsWith("data:image/")),
        )
      ) {
        return true;
      }
      if (
        ...
        msg.content.some((c) => c?.type === "image_url" || c?.image_url)
      ) {
        return true;
      }
      if (
        typeof msg?.content === "string" &&
        /data:image\/(?:png|jpeg|jpg|webp|gif);base64,/i.test(msg.content)
      ) {
        return true;
      }
    }
  ...
  if (
    ...
    skills.some((s) => VISUAL_SKILL_IDS.has(s?.id || s))
  ) {
    return true;
  }
  ...
  const combinedText =
    `${prompt} ${intent?.goal || ""} ${intent?.summary || ""}`.toLowerCase();
...
export function selectModelForRequest(options = {}, env = {}) {
  ...
  if (isVisual) {
    return (
      env?.OPENCODE_VISUAL_MODEL || env?.OPENCODE_BUILD_MODEL || MODEL_VISUAL
    );
  }

  return (
    env?.OPENCODE_LOGIC_MODEL ||
...
export function selectReasoningConfig(options = {}, env = {}) {
  ...
  const complexity = String(
    options.complexity ||
      options.intent?.complexity ||
      options.fineIntent?.complexity ||
      "medium",
  ).toLowerCase();
  const intentType = String(
    options.intent?.type ||
      options.fineIntent?.type ||
      options.primaryIntent ||
      "general",
  ).toLowerCase();
  const primaryIntent = String(
    options.primaryIntent ||
      options.intent?.primaryIntent ||
      options.fineIntent?.primaryIntent ||
      "",
  ).toLowerCase();
  const skillIds = Array.isArray(options.skills)
    ? options.skills.map((s) => (typeof s === "string" ? s : s.id))
    : [];
  ...
  const hasHighReasoningSkill = skillIds.some((id) =>
    [
      "research-report",
      "data-analysis",
      "live-data-utilities",
  ...
  if (hasHighReasoningSkill && ["low", "medium"].includes(effort))
    effort = "high";
  if (primaryIntent === "game_creation" && effort === "low") effort = "medium";

  // Temperature: intent takes precedence, then complexity
  let temperature;
  const skillTemp = skillIds.find(
    (id) => TEMPERATURE_BY_INTENT[id] !== undefined,
  );
  if (skillTemp) {
  ...
  if (
    ["writing", "creative-writing"].includes(intentType) &&
  ...
  if (
    ["code-help", "data-analysis"].includes(intentType) ||
    skillIds.includes("data-analysis")
  ) {
