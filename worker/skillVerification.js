...
export const LIVE_DATA_KINDS = [
  {
    kind: "currency",
    patterns: [
      /\b(convert|exchange)\b.{0,30}\b(currency|usd|php|eur|gbp|jpy|exchange rate|currency conversion)\b/i,
      /\b\d+(?:[\d,]*\.\d+)?\s+(?:php|usd|eur|gbp|jpy|krw|cad|aud|inr)\s+(?:to|in|into)\s+\w+\b/i,
    ],
  },
  {
    kind: "weather",
    patterns: [
      /\b(weather|forecast|temperature|humidity|wind speed)\b.{0,30}\b(today|now|right now|this week|tomorrow|current)?\b/i,
    ],
  },
  {
    kind: "stock",
    patterns: [
      /\b(stock price|share price|stock market|trading price|market cap)\b/i,
    ],
  },
  {
    kind: "crypto",
    patterns: [
      /\b(bitcoin|ethereum|btc|eth|crypto)\b.{0,20}\b(price|rate|value|cost)\b|\b(price|rate|value)\b.{0,20}\b(bitcoin|ethereum|btc|eth|crypto)\b/i,
    ],
  },
  {
    kind: "current-time",
    patterns: [
      /\bwhat('| i)?s?\s+the\s+current\s+time\b|\bcurrent time\b|\bwhat time is it (now|right now)\b/i,
    ],
  },
  {
    kind: "current-date",
    patterns: [
      /\bwhat('| i)?s?\s+(today'?s\s+)?date\b|\btoday'?s\s+date\b|\bwhat\s+day\s+is\s+it\b|\bwhat\s+year\s+is\s+it\b/i,
    ],
  },
  {
    kind: "sports",
    patterns: [
      /\b(match result|game result|scoreline)\b.{0,20}\b(team|match|game|league)\b|\bwho won\b.{0,20}\b(match|game|final|championship)\b/i,
    ],
  },
  {
    kind: "schedules",
    patterns: [
      /\b(flight schedule|train schedule|bus schedule|departure time|arrival time|showtimes|opening hours|business hours)\b/i,
    ],
  },
  {
    kind: "public-figures",
    patterns: [
      /\b(who is the (current|new) (president|prime minister|chancellor|mayor|governor)|current (president|prime minister) of)\b/i,
    ],
  },
  {
    kind: "pricing",
    patterns: [
      /\b(price of|prices? of|how much (is|does|are))\b.{0,40}\b(iphone|car|house|laptop|console|ticket|fare)\b/i,
    ],
  },
  {
    kind: "travel-info",
    patterns: [
      /\b(current (visa|entry|quarantine) (rules|requirements)|travel restrictions|visa requirements\s+(now|currently|this year))\b/i,
    ],
  },
  {
    kind: "laws",
    patterns: [
      /\b(current (law|regulation|legal age|tax rate)|minimum wage\s+(now|currently)|what is the legal age)\b/i,
    ],
  },
  {
    kind: "availability",
    patterns: [
      /\b(in stock|in stock now|available now|still available|release date|next release)\b/i,
    ],
  },
  ...
  for (const { kind, patterns } of LIVE_DATA_KINDS) {
    for (const pattern of patterns) {
      if (pattern.test(text))
        return { required: true, kind, matched: String(pattern) };
    }
  }
...
const TEMPORAL_CLAIM_PATTERNS = [
  {
    type: "date",
    pattern:
      /\b(?:as of|as at)\s+([A-Z][a-z]+ \d{1,2},? \d{4}|\d{4}-\d{2}-\d{2}|\d{1,2}\/\d{1,2}\/\d{4})/gi,
  },
  { type: "year", pattern: /\b(?:current year(?: is)?|as of)\s+(\d{4})\b/gi },
  {
    type: "today",
    pattern:
      /\btoday(?:'s)?\s+(?:date\s+)?is\s+([A-Z][a-z]+ \d{1,2},? \d{4}|\d{1,2}\/\d{1,2}\/\d{4})/gi,
  },
];
...
export function checkTemporalClaims(content, runtimeContext) {
  ...
  for (const { type, pattern } of TEMPORAL_CLAIM_PATTERNS) {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      if (type === "year") {
        ...
        if (parsed) {
          const sameDay =
            parsed.year === now.getUTCFullYear() &&
            parsed.month === currentMonth &&
            parsed.day === currentDay;
          if (!sameDay)
            failures.push(
              type === "date" ? "stale-live-data" : "fabricated-current-date",
            );
        }
      }
    }
  ...
  const timePattern =
    /\b(current time(?: is|:| is now)|time right now is)\s+([0-9]{1,2}:[0-9]{2})/gi;
...
export function calcStats(numbers) {
  ...
  for (let i = 1; i < numbers.length; i += 1) {
    ...
    pctChanges.push(
      prev !== 0 ? ((numbers[i] - prev) / Math.abs(prev)) * 100 : null,
    );
  }
...
export function verifyLiveData({
  prompt,
  content,
  runtimeContext,
  liveDataEvidence,
  ...
  if (!need.required)
    return { failures, evidence, liveData: { liveDataRequired: false } };
  ...
  const newNumericClaims = claimedNumbers.filter(
    (claim) =>
      !promptNumbers.some((given) => Math.abs(given - claim) < 0.000001),
  );
  ...
  const fetchedUrls = new Set(
    (liveDataEvidence?.results || []).map((result) => canonicalUrl(result.url)),
  );
  const hasSource = LIVE_SOURCE_URL_PATTERN.test(text);
  const hasGroundedSource = citedUrls.some((url) =>
    fetchedUrls.has(canonicalUrl(url)),
  );
  ...
  if (usedLive) {
    ...
    return {
      ...
      liveData: {
        liveDataRequired: true,
        liveDataUsed: true,
        freshnessValid: validTimestamp && freshnessMs <= maxAgeMs,
        ...evidence,
      },
    };
  }
  ...
  return {
    failures: Array.from(new Set(failures)),
    evidence,
    liveData: { liveDataRequired: true, liveDataUsed: false, ...evidence },
  };
}
...
export function extractCitations(content) {
  ...
  const bareUrls =
    textWithoutMarkdownLinks.match(/https?:\/\/[^\s)\]>]+/g) || [];
...
export function verifyResearch({ prompt, content, searchEvidence }) {
  ...
  const text = String(content || "");
  const askedForSources =
    /\b(cite|sources?|references?|latest|current|recent|as of)\b/i.test(
      String(prompt || ""),
    );

  const citations = extractCitations(text);
  const fetchedUrls = new Set(
    (searchEvidence?.results || []).map((r) => canonicalUrl(r.url)),
  );

  for (const citation of citations) {
    const valid = validCitationUrl(citation.url);
    const fetched =
      fetchedUrls.size > 0 && fetchedUrls.has(canonicalUrl(citation.url));
  ...
  const hasAnyGrounded =
    citations.length > 0 &&
    evidence.fetchedSources >= Math.ceil(citations.length * 0.5);
  if (askedForSources && citations.length === 0) {
    failures.push("unsupported-citation");
  }
  evidence.groundingValid = hasAnyGrounded;
  evidence.groundingMode = searchEvidence?.servedAt
    ? "external-research"
    : "model-knowledge";
...
export function extractActionItems(content) {
  ...
  const taskFirst =
    /task/.test(headerLower) &&
    /owner/.test(headerLower) &&
  ...
  const listMatch = text.match(
    /(?:#{1,6}\s*)?(?:Action Items?|Confirmed Action Items?)\s*:?\s*\n?([\s\S]*?)(?=\n\s*(?:#{1,6}\s+|\*\*[A-Za-z])|$)/i,
  );
  if (listMatch) {
    ...
    for (const bullet of bullets) {
      ...
      const boldMatch = bulletAction.match(
        /^\*\*([^*]+)\*\*\s*(?:—|-|:)?\s*([\s\S]*)$/,
      );
      if (boldMatch) {
        ...
        const dashMatch = bulletAction.match(
          /^([A-Z][A-Za-z ]+?)\s*(?:—|-)\s*(.+)$/,
        );
        if (dashMatch) {
...
export function verifyMeetingNotes({ prompt, content, sourceText }) {
  ...
  for (const action of actions) {
    ...
    const meaningfulDeadline =
      action.deadline &&
      !/^(?:not stated|not specified|none|n\/a|tbd|ongoing)$/i.test(
    ...
    if (meaningfulDeadline) {
      ...
      if (inSource && ownerLower) {
        ...
        nearOwner =
          ownerIndex >= 0 &&
          deadlineIndex >= 0 &&
          Math.abs(deadlineIndex - ownerIndex) <= 80;
      }
  ...
  const suggestedSection = text.match(
    /Suggested Follow-ups?:?\s*\n?([\s\S]*?)(?=\n\s*#{1,3}\s|\n\s*$)/i,
  );
  if (suggestedSection) {
    evidence.suggested = (
      suggestedSection[1].match(/[-*]\s+[^\n]+/g) || []
...
const PLACEHOLDER_MARKERS =
  /\[(?:optional|placeholder|insert|consider|suggested)[^\]]*\]|consider (?:offering|giving|adding|using)|suggest(?:ion|ed)?/i;
const NON_ASSERTIVE_CLAIM_CONTEXT =
  /\b(?:check|confirm|verify)\s+(?:whether|if|before)\b|\bif\s+you\s+offer\b|\byour\s+real\s+(?:offer|policy|claim|rate|details?)\b/i;
...
export function verifyMarketing({ _prompt, content }) {
  ...
  for (const pattern of FABRICATED_BUSINESS_CLAIM_PATTERNS) {
    ...
    if (match) {
      ...
      const claimIsBracketed = Array.from(
        withoutLinks.matchAll(/\[[^\]]*\]/g),
      ).some(
        (bracket) =>
          claimIndex >= bracket.index &&
          claimIndex < bracket.index + bracket[0].length,
      );
      if (
        PLACEHOLDER_MARKERS.test(line) ||
        NON_ASSERTIVE_CLAIM_CONTEXT.test(line) ||
        claimIsBracketed
      )
        continue;
  ...
  if (promptNumbers.length >= 2) {
    ...
    const check = (label, expectedValue, candidates, conditional) => {
      ...
      const match = candidates.find(
        (n) =>
          Math.abs(n - expectedValue) <=
          Math.max(0.01, Math.abs(expectedValue) * 0.01),
      );
      if (match === undefined) {
        failures.push(
          label === "sum"
    ...
    check(
      ...
      expected.sum,
      responseNumbers,
      /\btotal\b|\bsum of\b|\ball.?time\b/i.test(String(content || "")),
    );
    check("mean", expected.mean, responseNumbers);

    const text = String(content || "");
    const monthPercentagePattern =
    ...
    while ((percentageMatch = monthPercentagePattern.exec(text)) !== null) {
      ...
      if (
        Number.isFinite(expectedChange) &&
        Math.abs(Math.abs(expectedChange) - claimed) > 0.5
      ) {
        failures.push("percentage-error");
        evidence.checks.push({
          label: `month-${month}-percentage`,
          expected: round2(expectedChange),
          found: claimed,
        });
      }
    }
    ...
    const directionMatch = text.match(
      /\b(?:overall\s+)?trend\s+(?:is|was|looks?)\s+(upward|downward|flat|stable)\b/i,
    );
    if (directionMatch) {
      const actualDirection = numbersDirection(promptNumbers);
      const claimedDirection = /upward/i.test(directionMatch[1])
        ? "upward"
        : /downward/i.test(directionMatch[1])
          ? "downward"
          : "flat";
    ...
    const slopeMatch =
      text.match(
        /\blinear\s+trend\s+slope[^\d+-]{0,20}([-+]?[\d,]+(?:\.\d+)?)/i,
      ) ||
      text.match(
        /\bexpected\s+(?:increase|decrease)[^\d+-]{0,20}([-+]?[\d,]+(?:\.\d+)?)\s+per\s+(?:month|period)/i,
      );
    if (slopeMatch && trend) {
      const claimedSlope = Number(slopeMatch[1].replace(/,/g, ""));
      const signedSlope =
        /decrease/i.test(slopeMatch[0]) && claimedSlope > 0
          ? -claimedSlope
          : claimedSlope;
      const arithmetic = verifyArithmetic({
        expected: trend.slope,
        actual: signedSlope,
        tolerance: 0.01,
        failure: "trend-calculation-error",
      });
      failures.push(...arithmetic.failures);
      evidence.checks.push({
        label: "linear-trend-slope",
        expected: round2(trend.slope),
        found: signedSlope,
        ok: arithmetic.failures.length === 0,
      });
    }

    const formulaMatch = text.match(
      /(?:sales|trend|y)?\s*[≈~=]\s*([\d,]+(?:\.\d+)?)\s*\+\s*([\d,]+(?:\.\d+)?)\s*[×*x]\s*(?:month|x)/i,
    );
    const forecastMatch = text.match(
      /\bmonth\s+(\d+)\s+forecast[^\d]{0,20}([\d,]+(?:\.\d+)?)/i,
    );
    if (formulaMatch && forecastMatch) {
      ...
      evidence.checks.push({
        ...
        expected: round2(expectedForecast),
        found: forecast,
        ok: arithmetic.failures.length === 0,
      });
    }
...
export function verifyFinance({ prompt, content }) {
  ...
  if (promptNumbers.length > 0) {
    ...
    const found = responseNumbers.find(
      (n) =>
        Math.abs(n - expectedTotal) <=
        Math.max(0.01, Math.abs(expectedTotal) * 0.01),
    );
    if (found === undefined) {
      ...
      evidence.checks.push({
        ...
        found: responseNumbers.slice(0, 8),
      });
    } else {
      evidence.checks.push({
        label: "budget-total",
    ...
    for (const rows of tableGroups) {
      const headerIndex = rows.findIndex(
        (cells) =>
          cells.some((cell) => /category|line item/i.test(cell)) &&
          cells.some((cell) => /budget|amount|allocation/i.test(cell)),
      );
      if (headerIndex < 0) continue;
      const headers = rows[headerIndex];
      const amountIndex = headers.findIndex((cell) =>
        /budget|amount|allocation/i.test(cell),
      );
...
export function verifyBusiness({ _prompt, content }) {
  ...
  const text = String(content || "");
  const hasAssumptionLabels =
    /\b(assum(e|ing|ption)|illustrative|estimate|placeholder|example figure)\b/i.test(
      text,
    );
  evidence.assumptionsLabeled = hasAssumptionLabels;
  const hasFigures =
    /\b\d[\d,]*(?:\.\d+)?\s*(?:php|usd|eur|₱|€|\$|£|%)\b|\b(?:php|usd|eur|₱|€|\$|£)\s?\d[\d,]*/i.test(
      text,
    );
...
const CLOCK_TIME_PATTERN =
  /(\b(?:[01]?\d|2[0-3])(?::[0-5]\d)?\s?(?:am|pm)\b|\b(?:[01]?\d|2[0-3]):[0-5]\d\b)/gi;
...
const DAY_MARKER_PATTERN =
  /^\s*(?:#{1,6}\s*)?(?:[-*]\s+)?day\s+(one|two|three|four|five|six|\d{1,2})\b/i;
...
export function extractItinerary(content) {
  ...
  for (const line of lines) {
    ...
    if (
      /\b(start|leave|depart|arrive|go|begin|wake|book|get|head|catch|take|before|by)\b/i.test(
        timePrefix,
      )
    )
      continue;
    ...
    const between =
      tokens.length > 1
        ? line.slice(tokens[0].index + tokens[0].value.length, tokens[1].index)
        : "";
    ...
    const end =
      isRange && tokens.length > 1 ? parseClock(tokens[1].value) : null;
    ...
    const endHasPm =
      isRange && tokens.length > 1 && /pm/i.test(tokens[1].value);
    if (
      isRange &&
      start &&
      start.minutes < 7 * 60 &&
...
const DURATION_PATTERN =
  /(?:duration\s*[:=]?\s*|for\s+)?(\d+(?:\.\d+)?)\s*(?:-|–|to)\s*\d+(?:\.\d+)?\s*(hours?|hrs?)/i;
...
function parseDuration(line) {
  ...
  if (
    single &&
    /duration|for|tour|visit|walk|drive|ride|class|session|activity|hike|trek/i.test(
      line,
    )
  ) {
...
const LONG_TRANSFER_PATTERN =
  /\b(drive|ride|van|bus|flight|transfer|airport|port|hrs? (away|each way)|minutes? (away|each way))\b/i;
  ...
  for (let i = 0; i < events.length; i += 1) {
    ...
    evidence.events.push({
      ...
      startTime: event.start
        ? `${Math.floor(event.start.minutes / 60)}:${String(event.start.minutes % 60).padStart(2, "0")}`
        : null,
    ...
    if (event.start && i + 1 < events.length) {
      ...
      if (finish !== null && finish > next.start.minutes) {
        ...
        const embeddedStop =
          /(?:quick stop|stop (?:at|in|for)|on the way|en route|via|rest stop)/i.test(
            next.activity,
          );
      ...
      const buffer = LONG_TRANSFER_PATTERN.test(next.activity)
        ? TRAVEL_BUFFER_MINUTES
        : SHORT_BUFFER_MINUTES;
      if (
        finish !== null &&
        finish !== next.start.minutes &&
        finish + buffer > next.start.minutes
      ) {
...
export function verifyFitness({ prompt, content }) {
  ...
  const hasProgression =
    /\b(start|begin|week 1|beginner|build up|progress|gradually)\b/i.test(text);
...
export function validateAccessibilityHtml(content) {
  ...
  const htmlBlocks =
    String(content || "").match(/```(?:html|jsx)?\s*([\s\S]*?)```/g) || [];
  ...
  for (const html of blocks) {
    ...
    const labels = Array.from(html.matchAll(/<label[^>]*for="([^"]+)"/gi)).map(
      (m) => m[1],
    );
    ...
    const ariaRefs = Array.from(
      html.matchAll(
        /(aria-describedby|aria-labelledby|aria-controls)="([^"]+)"/gi,
      ),
    );
      ...
      const hasLabel =
        ...
        (() => {
          ...
          return Boolean(
            ...
            ids.includes(labelledby.split(/\s+/)[0]),
          );
        })() ||
...
const TIME_ASSIGNMENT_PATTERN =
  /\b(\d{1,2}:\d{2}\s?(?:am|pm)?|\b\d{1,2}\s?(?:am|pm))\b/i;
const ASSUMPTION_LABEL_PATTERN =
  /\b(assum(e|ing|ption)|placeholder|actual .* time|sample times|to be confirmed|tbd)\b|\[[^\]]*\]/i;

export function verifyProductivity({ prompt, content }) {
  const failures = [];
  const evidence = { assumptions: [] };
  const text = String(content || "");
  const userTimes = (String(prompt || "").match(TIME_ASSIGNMENT_PATTERN) || [])
    .length;
  ...
  if (userTimes === 0 && responseTimes > 0) {
    ...
    if (!labeled) {
      ...
      evidence.assumptions.push({
        field: "scheduleTime",
        userProvided: false,
        value: "unspecified clock time in response",
      });
    }
  }
  return { failures: Array.from(new Set(failures)), evidence };
...
export function runSkillVerification({
  prompt,
  content,
  skills,
  runtimeContext,
  ...
  for (const skillId of uniqueSkills) {
    ...
    try {
      ...
      outcome = {
        failures: [],
        evidence: { verifierError: String(err?.message || err).slice(0, 200) },
      };
    }
...
export function applyMeetingNotesPatches({ content, sourceText }) {
  ...
  const output = lines.flatMap((line) => {
    ...
    if (
      ...
      lower.some((cell) => /task|action/.test(cell)) &&
      lower.some((cell) => /due|deadline/.test(cell))
    ) {
      columns = {
    ...
    if (!columns || cells.every((cell) => /^:?-{3,}:?$/.test(cell)))
      return [line];

    const owner = String(cells[columns.owner] || "")
      .replace(/[*_`]/g, "")
      .trim()
      .toLowerCase();
    const task = String(cells[columns.task] || "")
      .replace(/[*_`]/g, "")
      .trim()
      .toLowerCase();
    const deadline = String(cells[columns.deadline] || "")
      .replace(/[*_`]/g, "")
      .trim()
      .toLowerCase();
    ...
    const nearbyTask = taskTokens.some((token) => {
      ...
      return (
        tokenIndex >= 0 &&
        ownerIndex >= 0 &&
        Math.abs(tokenIndex - ownerIndex) <= 80
      );
    });
    ...
    const meaningfulDeadline =
      deadline &&
      !/^(?:not stated|not specified|none|n\/a|tbd|ongoing)$/.test(deadline);
    if (meaningfulDeadline) {
      const deadlineIndex = source.lastIndexOf(deadline);
      const deadlineGrounded =
        deadlineIndex >= 0 &&
        (!owner ||
          (ownerIndex >= 0 && Math.abs(deadlineIndex - ownerIndex) <= 80));
      if (!deadlineGrounded) {
...
export function runVerificationWithRepair({
  prompt,
  content,
  skills,
  runtimeContext,
  ...
  while (attempts < MAX_REPAIR_ATTEMPTS) {
    ...
    const skillIds = (Array.isArray(skills) ? skills : []).map((skill) =>
      typeof skill === "string" ? skill : skill.id,
    );
    const meetingFailures = verdict.hardFailures.some((failure) =>
      [
        "fabricated-action-item",
        "fabricated-owner",
        "fabricated-deadline",
    ...
    if (meetingFailures && skillIds.includes("meeting-notes")) {
      patched = applyMeetingNotesPatches({
        content: current,
