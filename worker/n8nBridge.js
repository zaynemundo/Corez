...
function getWebhookUrl(requestBody, env) {
  const fromBody = typeof requestBody?.webhookUrl === 'string' ? requestBody.webhookUrl.trim() : '';
  if (fromBody && /^https?:\/\//i.test(fromBody)) return fromBody;
  const fromEnv = typeof env?.N8N_WEBHOOK_URL === 'string' ? env.N8N_WEBHOOK_URL.trim() : '';
...
export async function handleN8nWebhook(request, env) {
  ...
  if (request.method !== 'POST') {
    return jsonResponse(405, { error: 'Method not allowed. Use POST /api/n8n/webhook' });
  }
  ...
  if (env?.AUTH_SECRET) {
    ...
    if (!sess) return jsonResponse(401, { error: 'Authentication required for n8n webhook bridge.' });
  }
  ...
  if (!webhookUrl) {
    return jsonResponse(400, { error: 'Missing webhookUrl. Provide { webhookUrl } in body or set N8N_WEBHOOK_URL in worker env.' });
  }
  if (!isAllowedWebhookUrl(webhookUrl)) {
    return jsonResponse(400, { error: 'Invalid webhookUrl — must be http(s) URL.' });
  }

  const event = typeof body?.event === 'string' ? body.event.slice(0, 120) : 'corez.event';
  const payload = body?.payload !== undefined ? body.payload : (body?.data !== undefined ? body.data : {});
  const forwarded = {
    event,
    source: 'corez',
    timestamp: Date.now(),
  ...
  const timeoutMs = Math.min(20000, Math.max(1000, Number(env?.N8N_WEBHOOK_TIMEOUT_MS) || DEFAULT_TIMEOUT_MS));
  ...
  try {
    ...
    if (!res.ok) {
      return jsonResponse(502, { error: `n8n webhook responded HTTP ${res.status}`, detail: safeErrorDetail(text).slice(0, 500), forwardedTo: webhookUrl });
    }
    return jsonResponse(200, { success: true, forwardedTo: webhookUrl, event, n8nResponse: data });
  } catch (e) {
    clearTimeout(timeout);
    const isAbort = e?.name === 'AbortError';
    return jsonResponse(isAbort ? 504 : 502, { error: isAbort ? `n8n webhook timeout after ${timeoutMs}ms` : 'Failed to reach n8n webhook', detail: safeErrorDetail(e).slice(0, 500), forwardedTo: webhookUrl });
  }
}
