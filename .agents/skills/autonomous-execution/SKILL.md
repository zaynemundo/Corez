...
- **Path containment**: Never target system directories outside the workspace. Always use paths within the workspace root; on Windows, quote any path containing spaces.
...
- **Write-test-fix loop**: Apply changes in small, logical chunks. Immediately after modifying code:
  1. Run the project's test suite or test helper (`npm test`).
  2. Run the linter or compiler (`npm run build`, `npm run lint`) to catch syntax/type issues early.
...
- **Optimize reads**: Avoid reading entire large files. Use the Read tool with offset/limit windows to fetch only the relevant lines.
